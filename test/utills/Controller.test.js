...
contract("Controller", async () => {
  before("setup1", async () => {
    ...
    tezos.setSignerProvider(
      await new InMemorySigner.fromSecretKey(accountsMap.get(accounts[0]))
    );
    ...
    operation = await gInstance.methods
      .updReturnAddressOracle(cInstance.address)
      .send();
    await confirmOperation(tezos, operation.hash);
  ...
  async function setup2(name) {
    ...
    const operation = await fInstance.methods
      .launchToken(name, XTZInstance.address)
      .send();
    await confirmOperation(tezos, operation.hash);
    ...
    const operation2 = await XTZInstance.methods
      .approve(qTokenAddress, 10000000)
      .send();
    await confirmOperation(tezos, operation2.hash);

    tezos.setSignerProvider(
      await new InMemorySigner.fromSecretKey(accountsMap.get(accounts[1]))
    );
    const operation3 = await XTZInstance.methods
      .approve(qTokenAddress, 10000000)
      .send();
    await confirmOperation(tezos, operation3.hash);

    tezos.setSignerProvider(
      await new InMemorySigner.fromSecretKey(accountsMap.get(accounts[2]))
    );
    const operation4 = await XTZInstance.methods
      .approve(qTokenAddress, 10000000)
      .send();
    ...
    // oraclePrice += 100/
    var oracleOperation = await gInstance.methods
      .updParamsOracle(oraclePrice, startTimestamp)
      .send();
    await confirmOperation(tezos, oracleOperation.hash);

    oracleOperation = await cInstance.methods
      .useController("sendToOracle", qTokenAddress)
      .send();
    await confirmOperation(tezos, oracleOperation.hash);
  ...
  describe("setOracle", async () => {
    it("set new oracle colled by admin", async () => {
      ...
      const operation = await cInstance.methods
        .useController("setOracle", oracle)
        .send();
      await confirmOperation(tezos, operation.hash);
    ...
    it("set new oracle colled by non-admin", async () => {
      ...
      await rejects(
        cInstance.methods.useController("setOracle", oracle).send(),
        (err) => {
          ok(err.message == "NotAdmin", "Error message mismatch");
  ...
  describe("safeMint", async () => {
    it("Safe Mint 140 for account 0; qToken non contain in qTokens list", async () => {
      ...
      await rejects(
        cInstance.methods
          .useController("safeMint", amount, randomAddress)
          .send(),
        (err) => {
          ok(err.message == "qTokenNotContainInSet", "Error message mismatch");
    ...
    it("Safe Mint 140 for account 0; qToken contain in qTokens list", async () => {
      ...
      var amount = 1000000;
      const operation = await cInstance.methods
        .useController("safeMint", amount, qTokens[qTokens.length - 2])
        .send();
      await confirmOperation(tezos, operation.hash);
  ...
  describe("safeMint2", async () => {
    it("Safe Mint 150 for account 1", async () => {
      ...
      var amount = 1000000;
      const operation = await cInstance.methods
        .useController("safeMint", amount, qTokens[qTokens.length - 1])
        .send();
      await confirmOperation(tezos, operation.hash);
  ...
  describe("safeBorrow", async () => {
    it("Safe Borrow 10 for account 0; qToken non contain in qTokens list", async () => {
      ...
      await rejects(
        cInstance.methods
          .useController("safeBorrow", randomCollateral, amount, randomBorrow)
          .send(),
        (err) => {
          ok(err.message == "qTokenNotContainInSet", "Error message mismatch");
    ...
    it("Safe Borrow 10 for account 0. Brower token is`nt minted", async () => {
      ...
      await rejects(
        cInstance.methods
          ...
          .send(),
        (err) => {
          ok(err.message == "DIV by 0", "Error message mismatch");
    ...
    it("Safe Borrow 10 for account 0", async () => {
      ...
      console.log(
        "Account MEM: ",
        await MStorage.storage.accountMembership.get(accounts[0])
      );
      ...
      console.log(
        "Acc Borr in Contr: ",
        (await MStorage.storage.accountBorrows.get(arr)).toString()
      );
    });
    ...
    it("Safe Borrow 10 for account 0. CollateralToken already entered to market", async () => {
      ...
      await rejects(
        cInstance.methods
          ...
          .send(),
        (err) => {
          ok(err.message == "AlreadyEnteredToMarket", "Error message mismatch");
  ...
  describe("safeRedeem", async () => {
    it("Safe Redeem 20 for account 0. qToken non contain in qTokens list", async () => {
      ...
      await setup2("CAKE-USD"); // 0,1,2,3,4
      var amount = 500000;
      var randomToken = "KT1RCNpUEDjZAYhabjzgz1ZfxQijCDVMEaTZ";

      await rejects(
        cInstance.methods
          .useController("safeRedeem", amount, randomToken)
          .send(),
        (err) => {
          ok(err.message == "qTokenNotContainInSet", "Error message mismatch");
    ...
    it("Safe Redeem 20 for account 1. Redeem without mint.", async () => {
      ...
      await rejects(
        cInstance.methods
          .useController("safeRedeem", amount, qTokens[qTokens.length - 5])
          .send(),
        (err) => {
          ok(err.message == "NotEnoughTokensToBurn", "Error message mismatch");
  ...
  describe("safeRepay", async () => {
    it("Safe Repay 10 for account 0. qToken non contain in qTokens list", async () => {
      ...
      await rejects(
        cInstance.methods
          .useController("safeRepay", amount, randomToken)
          .send(),
        (err) => {
          ok(err.message == "qTokenNotContainInSet", "Error message mismatch");
    ...
    it("Safe Repay 10 for account 1. user doesnt have borrow", async () => {
      ...
      await rejects(
        cInstance.methods
          .useController("safeRepay", amount, qTokens[qTokens.length - 4])
          .send(),
        (err) => {
          ok(
  ...
  describe("exitMarket", async () => {
    it("remove accountMembership for account 0. Borrow exist", async () => {
      ...
      await rejects(
        cInstance.methods.useController("exitMarket", accounts[0]).send(),
        (err) => {
          ok(err.message == "BorrowsExists", "Error message mismatch");
    ...
    it("remove accountMembership for account 1. user doesnt entered to market", async () => {
      ...
      await rejects(
        cInstance.methods.useController("exitMarket", accounts[1]).send(),
        (err) => {
          ok(
            err.message == "CantGetUpdateControllerStateEntrypoint",
            "Error message mismatch"
    ...
    it("remove accountMembership for account 0. Borrow not exist", async () => {
      ...
      var operation = await cInstance.methods
        .useController("safeRepay", amount, qTokens[qTokens.length - 4])
        .send();
      await confirmOperation(tezos, operation.hash);
      ...
      arr = [accounts[0], qTokens[qTokens.length - 4]];
      console.log(
        "Acc Borr in Contr after: ",
        (await MStorage.storage.accountBorrows.get(arr)).toString()
      );

      operation = await cInstance.methods
        .useController("exitMarket", accounts[0])
        .send();
      await confirmOperation(tezos, operation.hash);
  ...
  describe("safeMint3 for SafeLiquidate", async () => {
    it("Safe Mint 150 for account 2", async () => {
      ...
      await setup2("BUNNY-USD"); // 0,1,2,3,4,5
      ...
      var amount2 = 160;
      const operation2 = await cInstance.methods
        .useController("safeMint", amount2, qTokens[qTokens.length - 1])
        .send();
      await confirmOperation(tezos, operation2.hash);
  ...
  describe("safeBorrow2 for SafeLiquidate", async () => {
    it("Safe Borrow 20 for account 2", async () => {
      ...
      console.log(
        "Account MEM: ",
        await MStorage.storage.accountMembership.get(accounts[2])
      );
      ...
      console.log(
        "Account MEM0: ",
        await MStorage2.storage.accountMembership.get(accounts[0])
      );
      const arr2 = [accounts[0], qTokens[qTokens.length - 6]];

      console.log(
        "Acc Borr0: ",
        (await MStorage2.storage.accountBorrows.get(arr2)).toString()
      );
    });
  });
  ...
  describe("safeLiquidate", async () => {
    it("Safe Liquidate. qToken not in qTokens list", async () => {
      ...
      await rejects(
        cInstance.methods
          .useController("safeLiquidate", borrower, amount, randomToken)
          .send(),
        (err) => {
          ok(err.message == "qTokenNotContainInSet", "Error message mismatch");
    ...
    it("Safe Liquidate.Brower debt amount is zero", async () => {
      ...
      await rejects(
        cInstance.methods
          ...
          .send(),
        (err) => {
          ok(err.message == "DebtIsZero", "Error message mismatch");
    ...
    it("Safe Liquidate", async () => {
      ...
      console.log(
        "Acc Tok controller: ",
        (await MStorage.storage.accountTokens.get(arr)).toString()
      );
      console.log(
        "Account MEM controller: ",
        await MStorage.storage.accountMembership.get(accounts[2])
      );
