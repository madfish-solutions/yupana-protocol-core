...
const ExchangeRate = async (s) => {
  ...
  const utilizationRateFloat = Floor(
    s.totalBorrows
      .multipliedBy(accuracy)
      .div(s.totalLiquid.plus(s.totalBorrows).minus(s.totalReserves))
  ); // one div operation with float require accuracy mult
  const borrowRatePerSecFloat = Floor(
    utilizationRateFloat
      .multipliedBy(utilizationBasePerSecFloat)
      .div(accuracy)
      .plus(debtRatePerSecFloat)
  ); // one mult operation with float require accuracy division
  const simpleInterestFactorFloat = Floor(
    borrowRatePerSecFloat.multipliedBy(
  ...
  const interestAccumulatedFloat = Floor(
    simpleInterestFactorFloat.multipliedBy(s.totalBorrows).div(accuracy)
  ); // one mult operation with float require accuracy division

  s.totalBorrows = interestAccumulatedFloat.plus(s.totalBorrows);
  s.totalReserves = Floor(
    interestAccumulatedFloat
      .multipliedBy(reserveFactorFloat)
      .div(accuracy)
      .plus(s.totalReserves)
  ); // one mult operation with float require accuracy division
  s.borrowIndex = Floor(
    simpleInterestFactorFloat
      .multipliedBy(s.borrowIndex)
      .div(accuracy)
      .plus(s.borrowIndex)
  );

  const exchangeRate = s.totalLiquid
    .plus(s.totalBorrows)
    .minus(s.totalReserves)
    .multipliedBy(accuracy)
    .div(s.totalSupply);
  return exchangeRate;
};
...
contract.skip("qToken", async () => {
  ...
  const totalBorrows = accuracy.multipliedBy(1e5);
  const totalLiquid = accuracy.multipliedBy(1e5);
  const totalSupply = accuracy.multipliedBy(1e5);
  const totalReserves = accuracy.multipliedBy(1e5);
  const borrowIndex = accuracy.multipliedBy(1e5);
  const accountBorrows = MichelsonMap.fromLiteral({
    [DEFAULT]: {
      amount: Fixed(accuracy.multipliedBy(1e5)),
      lastBorrowIndex: Fixed(accuracy.multipliedBy(1e5)),
    },
  });
  const accountTokens = MichelsonMap.fromLiteral({
    [DEFAULT]: Fixed(accuracy.multipliedBy(1e5)),
  });
  ...
  describe("deploy", async () => {
    it("should check storage after deploy", async () => {
      ...
      assert.equal(
        accountBorrows.get(DEFAULT).lastBorrowIndex,
        Fixed(actual.lastBorrowIndex)
      );
      assert.equal(
        accountTokens.get(DEFAULT),
        Fixed(await qTokenStorage.accountTokens.get(DEFAULT))
      );
    });
  });
  ...
  describe("setAdmin", async () => {
    ...
    it("should get exception, call from not owner", async () => {
      ...
      await truffleAssert.fails(
        qTokenInstance.setAdmin(newAdmin),
  ...
  describe("setOwner", async () => {
    ...
    it("should get exception, call from not owner", async () => {
      ...
      await truffleAssert.fails(
        qTokenInstance.setOwner(newOwner),
        truffleAssert.INVALID_OPCODE,
        "NotOwner"
      );
    });
  ...
  describe("mint", async () => {
    beforeEach("setup", async () => {
      ...
      await XTZ_Instance.approve(qTokenInstance.address, 1e5);
      await revertDefaultSigner();
    });
    it("should mint tokens", async () => {
      ...
      assert.equal(
        Fixed(mintTokens),
        Fixed(await qTokenStorage.accountTokens.get(RECEIVER))
      );
      assert.equal(
        Fixed(amount.multipliedBy(accuracy).plus(totalLiquid)),
        Fixed(qTokenStorage.totalLiquid)
      );
      assert.equal(
        Fixed(mintTokens.plus(totalSupply)),
        Fixed(qTokenStorage.totalSupply)
      );
      assert.equal(
        amount.toString(),
        (
          await (
            await XTZ_Instance.storage()
          ).ledger.get(qTokenInstance.address)
        ).balance
      );
    });
  });
  ...
  describe("redeem", async () => {
    ...
    beforeEach("setup, mint 100 tokens on receiver", async () => {
      ...
      accTokensAferMint = Fixed(
        await storageAfterMint.accountTokens.get(RECEIVER)
      );
    ...
    it("should redeem amount", async () => {
      const amountOfXTZbefore = (
        await (await XTZ_Instance.storage()).ledger.get(RECEIVER)
      ).balance;
      ...
      assert.equal(
        accTokensAferMint - burnTokens,
        Fixed(await qTokenStorage.accountTokens.get(RECEIVER))
      );
      assert.equal(
        Fixed(_totalSupply.minus(burnTokens)),
        Fixed(qTokenStorage.totalSupply)
      );
      assert.equal(
        Fixed(_totalLiquid.minus(amount.multipliedBy(accuracy))),
        Fixed(qTokenStorage.totalLiquid)
      );
      assert.equal(
        Fixed(amountOfXTZbefore.plus(amount)),
    ...
    it("should redeem all users tokens, pass 0 as amount", async () => {
      ...
      const usersTokens = await (
        await qTokenInstance.storage()
      ).accountTokens.get(RECEIVER);
    ...
    it("should redeem 50 tokens", async () => {
      ...
      assert.equal(
        usersTokens - burnTokens,
        await qTokenStorage.accountTokens.get(RECEIVER)
      );
      assert.equal(
        Fixed(_totalSupply.minus(burnTokens)),
        Fixed(qTokenStorage.totalSupply)
      ...
      assert.equal(
        _totalLiquid - amountTo * accuracy,
        qTokenStorage.totalLiquid
      );
      assert.equal(
        Fixed(amountTo.plus(amountOfXTZbefore)),
        (await (await XTZ_Instance.storage()).ledger.get(RECEIVER)).balance
      );
    });
    it("should get exception, exchange rate is zero", async () => {
      ...
      s.totalReserves = 1;
      s.borrowIndex = 1;
      s.totalSupply = Fixed(toBN(1e228));
      let q = await qToken.new(s);
      await truffleAssert.fails(
        q.redeem(RECEIVER, 0),
        truffleAssert.INVALID_OPCODE,
        "NotEnoughTokensToSendToUser"
      );
    });
  });
  ...
  describe("borrow", async () => {
    beforeEach("setup, add balance of xtz to qToken", async () => {
      await setSigner(RECEIVER);
      await XTZ_Instance.transfer(
        RECEIVER,
        qTokenInstance.address,
        receiverBalance
      );
      await revertDefaultSigner();
    });
    it("should borrow tokens", async () => {
      ...
      assert.equal(
        Fixed(storage.totalBorrows.plus(amount.multipliedBy(accuracy))),
        Fixed(qTokenStorage.totalBorrows)
      );
      assert.equal(
        amount.toNumber(),
    ...
    it("should get exception, total liquid less than amount", async () => {
      ...
      await truffleAssert.fails(
        qTokenInstance.borrow(RECEIVER, amount),
        truffleAssert.INVALID_OPCODE,
        "AmountTooBig"
      );
    });
  ...
  describe("repay", async () => {
    ...
    beforeEach(
      "setup, add balance of xtz to DEFAULT and make borrow to user",
      async () => {
        await setSigner(RECEIVER);
        await XTZ_Instance.transfer(
          RECEIVER,
    ...
    it("should repay tokens", async () => {
      const borrowsBeforeRepay = await (
        await qTokenInstance.storage()
      ).accountBorrows.get(RECEIVER);
      ...
      let borrowsAmount = toBN(borrowsBeforeRepay.amount)
        .multipliedBy(storageAfterBorrow.borrowIndex)
        .div(borrowsBeforeRepay.lastBorrowIndex)
        .div(accuracy);
      borrowsAmount = borrowsAmount.minus(
        accuracy.multipliedBy(amountToBorrow)
      );

      assert.equal(
        Math.abs(Fixed(borrowsAmount)),
        Fixed(_accountBorrows.amount)
      );
      // accuracy division because there is an infelicity in the calculations
      assert.equal(
        Floor(storageAfterBorrow.totalBorrows)
          .minus(accuracy.multipliedBy(amountToBorrow))
          .div(accuracy),
  ...
  describe("liquidate", async () => {
    ...
    beforeEach("setup, borrow 100 tokens on receiver and users", async () => {
      ...
      const seizeTokens = amountToBorrow
      ...
      borrowsAmount = toBN(borrow.amount)
        .multipliedBy(setupStorage.borrowIndex)
        .div(borrow.lastBorrowIndex);
      borrowsAmount = borrowsAmount.minus(seizeTokens);
    });
    it("should liquidate borrow", async () => {
      assert.equal(
        amountToBorrow * accuracy,
        (await (await qTokenInstance.storage()).accountBorrows.get(RECEIVER))
          .amount
      );
      ...
      // accuracy division because there is an infelicity in the calculations
      assert.equal(
        Math.floor(borrowsAmount.div(accuracy)),
        Math.floor(
          toBN(
            (
              await (
    ...
    it("should get exception, borrower is liquidator", async () => {
      await truffleAssert.fails(
        qTokenInstance.liquidate(LIQUIDATOR, LIQUIDATOR, amountToBorrow),
        truffleAssert.INVALID_OPCODE,
        "BorrowerCannotBeLiquidator"
      );
    });
