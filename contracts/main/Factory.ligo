...
[@inline] function getControllerContract(
  const controllerAddress: address)
                        : contract(iController) is
  case (
    Tezos.get_entrypoint_opt("%register", controllerAddress)
                        : option(contract(iController))
  ) of
    Some(contr) -> contr
    | None -> (
      failwith("CantGetContractToken") : contract(iController)
    )
  end;

function setFactoryAdmin(
...
function launchToken(
  ...
  block {
    ...
    const res : (operation * address) = createContr(
      (None : option(key_hash)), 0mutez, fullStorage
    );
    ...
    s.tokenList[token] := (res.1);
      ...
      Tezos.transaction(
        QRegister(
          record[token = token; qToken = res.1; pairName = oralcePairName]
        ),
        0mutez,
...
function main (
  const p               : factoryAction;
  const s               : factoryStorage)
                        : fullFactoryReturn is
  case p of
    | LaunchToken(params)           -> launchToken(
                                        params.token,
                                        params.oralcePairName,
                                        s
                                      )
    | SetFactoryAdmin(params)       -> setFactoryAdmin(params, s)
    | SetNewOwner(params)           -> setNewOwner(params,s)
    | SetTokenFunction(params)      -> setTokenFunction(
                                        params.index,
                                        params.func,
                                        s
                                      )
    | SetUseFunction(params)        -> setUseFunction(
                                        params.index,
                                        params.func,
                                        s
                                      )
